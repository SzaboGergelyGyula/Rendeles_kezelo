...
import {
  ...
  IDeleteTableArgs,
  IUpdateTableArgs
} from '../../preload/interfaces/table'

ipcMain.handle('create-table', (event, args: ICreateTableArgs) => {
...
ipcMain.handle('get-all-tables', (event) => {
...
ipcMain.handle('update-table', async (event, args: IUpdateTableArgs) => {
...
ipcMain.handle('delete-table', async (event, args: IDeleteTableArgs) => {
  const { tableId } = args
